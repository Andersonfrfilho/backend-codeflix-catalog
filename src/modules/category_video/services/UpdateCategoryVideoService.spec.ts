...
describe('CreatePostService', () => {
  ...
  it('should be able to create a new image and new user, after image', async () => {
    ...
    const updatePostServiceSpy = jest.spyOn(updatePostService, 'execute');

    const fakePostsRepositoryUpdateSpy = jest.spyOn(
      fakePostsRepository,
      'update',
    );
    const image_content = {
    ...
    const post_content_change = {
      content: faker.random.words(),
      published: faker.random.boolean(),
    };

    const post_change = await updatePostService.execute({
      author_id: user.id,
      id: post.id,
      ...post_content_change,
    });

    expect(post_change).toEqual(
      expect.objectContaining({
        id: expect.any(String),
        content: expect.any(String),
        author_id: expect.any(String),
    ...
    expect(updatePostServiceSpy).toHaveBeenCalled();
    expect(fakeUsersRepositoryFindByEmailSpy).toHaveBeenCalled();
    expect(fakePostsRepositoryUpdateSpy).toHaveBeenCalled();
    expect(fakeCacheProviderInvalidateByKeySpy).toHaveBeenCalled();
  });
});
