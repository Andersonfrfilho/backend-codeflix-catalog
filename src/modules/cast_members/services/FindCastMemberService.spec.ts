...
describe('FindPostService', () => {
  ...
  it('should be able to create a new image and new user, after image after find', async () => {
    ...
    const findPostServiceSpy = jest.spyOn(findPostService, 'execute');
    const fakePostsRepositoryFindSpy = jest.spyOn(
      fakePostsRepository,
      'findById',
    );
    const image_content = {
    ...
    const post_find = await findPostService.execute({ id: post.id });
    ...
    expect(fakePostsRepositoryFindSpy).toHaveBeenCalled();
  });
});
